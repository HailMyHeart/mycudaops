...
// 用宏定义替代原本的模板参数，确保共享内存和寄存器在编译期能固定大小
#define BM 128
#define BN 128
#define BK 8
#define TM 8
...
__global__ void gemm(int M, int N, int K, const float alpha, const float beta, float* A, float* B, float* C){
    ...
    do {
        k += BK;
        if (k < K) {
            // 异步预取下一轮大 Tile 的 Global Memory 数据
            FLOAT4_VAL(a_tile_reg[0]) = FLOAT4_VAL(A[OFFSET(a_tile_row, a_tile_col, K)]);
            FLOAT4_VAL(b_tile_reg[0]) = FLOAT4_VAL(B[OFFSET(b_tile_row, b_tile_col, N)]);
            A += BK; B += BK * N;
        }
        ...
        if (k < K) {
            ...
            // 为下一轮大循环铺垫，提前加载下个大 Tile 的第 0 层到 Fragment
        ...
        // 消耗掉当前大 Tile 的最后一层（bk = BK - 1，即已经在 a_frag[1] 中的缓存）
        #pragma unroll
        for (int j = 0; j < TM; j++) {
            #pragma unroll
            for (int l = 0; l < TN; l++) {
